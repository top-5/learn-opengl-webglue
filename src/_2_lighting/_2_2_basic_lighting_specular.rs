...
use cgmath::{Matrix4, vec3, Vector3, Point3, Deg, perspective};
use cgmath::prelude::*;
...
thread_local! {
    static STATE: RefCell<Option<State_2_2_2>> = RefCell::new(None);
}

pub unsafe fn reset_2_2_2() {
    STATE.with(|state| {
...
unsafe fn init_2_2_2() {
    ...
    STATE.with(|state| {
        *state.borrow_mut() = Some(State_2_2_2 {
            lighting_shader,
            lamp_shader,
            cube_vao: cubeVAO,
            light_vao: lightVAO,
