...
thread_local! {
    static STATE: RefCell<Option<State_2_5_2>> = RefCell::new(None);
}

pub unsafe fn reset_2_5_2() {
    STATE.with(|state| {
...
unsafe fn init_2_5_2() {
    ...
    STATE.with(|state| {
        *state.borrow_mut() = Some(State_2_5_2 {
            lighting_shader,
            lamp_shader,
            cube_vao,
            light_vao,
...
unsafe fn render_2_5_2() {
    ...
    STATE.with(|state| {
        if let Some(ref s) = *state.borrow() {
            ...
            s.lighting_shader.setFloat(c_str!("light.constant"), 1.0);
            s.lighting_shader.setFloat(c_str!("light.linear"), 0.09);
            s.lighting_shader.setFloat(c_str!("light.quadratic"), 0.032);
            ...
            let model = Matrix4::from_translation(s.light_pos) * Matrix4::from_scale(0.2);
