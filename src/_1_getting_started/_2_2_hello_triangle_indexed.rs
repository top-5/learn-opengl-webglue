...
// State for indexed triangle example
struct State_2_2 {
...
thread_local! {
    static STATE: RefCell<Option<State_2_2>> = RefCell::new(None);
}

/// Reset state (force re-initialization)
pub unsafe fn reset_2_2() {
    STATE.with(|state| {
...
unsafe fn init_2_2() {
    ...
    STATE.with(|state| {
        *state.borrow_mut() = Some(State_2_2 {
...
unsafe fn render_2_2() {
    STATE.with(|state| {
        if let Some(ref s) = *state.borrow() {
            gl::ClearColor(0.2, 0.3, 0.3, 1.0);
            gl::Clear(gl::COLOR_BUFFER_BIT);

            // draw rectangle using indexed drawing
